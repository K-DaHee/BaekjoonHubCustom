 ...
 * 예: BOJ_1001.java → BOJ_1001_2.java → BOJ_1001_3.java
 * @param {Array} existingFiles - 트리에 존재하는 파일 목록
 * @param {string} directory - 파일 디렉토리
 * @param {string} baseFileName - 기본 파일명 (예: BOJ_1001.java)
 * @returns {string} 사용할 파일명
 */
function getNextFileName(existingFiles, directory, baseFileName) {
  const dotIndex = baseFileName.lastIndexOf('.');
  const nameWithoutExt = baseFileName.substring(0, dotIndex); // BOJ_1001
  const extension = baseFileName.substring(dotIndex);           // .java
...
function makeAppendPRBody(bojData) {
  // prBody에서 이미 치환된 접근방법/어려웠던점/배운점 값은 bojData.prBody에 포함되어 있음
  // 여기서는 축약 형태로 별도 섹션 생성
  const months = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];
  const now = new Date();
  const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;

  // bojData.prBody에서 각 섹션의 내용을 추출
